...
- ⌨️ **<kbd>CapsLock</kbd> 一键说话。** 按住录音，松开自动识别并把结果打到当前光标位置。
...
```
┌─────────────────────────────────────────────────────────┐
│  全局快捷键 (CapsLock / 鼠标侧键 / F13..F16)             │
│                        │                                │
│                        ▼                                │
│  ┌─────────────┐  ┌──────────────┐  ┌───────────────┐  │
│  │ PortAudio   │─▶│  Paraformer  │─▶│  hot-rule +   │  │
│  │ 16 kHz 单声道│  │  sherpa-onnx │  │  hot.txt RAG  │  │
│  └─────────────┘  └──────────────┘  └───────┬───────┘  │
│                                              ▼          │
│                                    ┌─────────────────┐  │
│                                    │  SendInput 到    │  │
...
3. **摊平模型目录**到 `models/paraformer/`。注意：拷进去的是解压后目录**里面的内容**，而不是那个目录本身：
...
4. **双击 `VoxPen.App.exe`。** 任务栏右下角出现一个蓝底 "C" 托盘图标；首次模型加载约 2–3 秒。
5. **鼠标点到任意可输入位置，按住 <kbd>CapsLock</kbd> 说话，松开自动上屏。**
...
**热重载：** `config.json` / `hot-rule.txt` / `hot.txt` 修改后 3 秒自动生效。快捷键、模型路径、日记根目录的改动仍需重启。
